...
public class ProjectInviteController {
      ...
      public ArrayList<HBProjectInvitationComponent> loadComponents() throws IOException {
            ...
            if (loadValues()) {
                  for (int i = 0; i < this.listInvites.size(); i++) {
      ...
      public boolean loadValues() {
            boolean y = DB_OPERATION
                        .QUERY("FROM PROJECT_MEMBER WHERE MBR_PROF_COD = :COD AND MBR_INVITE_STATUS='ON_HOLD'", "COD",
            ...
            this.listInvites = (ArrayList<?>) DB_OPERATION.QUERY(
                        "FROM PROJECT_MEMBER WHERE MBR_COD_PROF = :COD AND MBR_INVITE_STATUS='ON_HOLD'", "COD",
                        SESSION.getProfileLogged().getCod());

            int[] profiles = new int[0];
            int[] projects = new int[0];
            ...
            this.listProjects = (ArrayList<?>) DB_OPERATION.QUERY("FROM PROJECT WHERE PROJ_COD  = :COD ", "COD",
                        projects);
            this.listProfiles = (ArrayList<?>) DB_OPERATION.QUERY("FROM USER_PROFILE WHERE PROF_COD  = :COD", "COD",
                        profiles);
