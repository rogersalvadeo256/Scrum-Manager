...
public class QUERYs_FRIENDSHIP {
	...
	public static List<USER_PROFILE> friendsList() {
		...
		for (FRIENDSHIP friend : (List<FRIENDSHIP>) listFriends) {
			List<?> profileList = DB_OPERATION.QUERY("FROM USER_PROFILE WHERE PROF_COD = :COD OR PROF_COD = :COD2",
					new String[] { "COD", "COD2" }, new int[] { friend.getRequestedBy(), friend.getReceiver() });
